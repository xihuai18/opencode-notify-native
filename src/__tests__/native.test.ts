...
async function createMacSoundFixture(
  ...
  const filePath = path.join(homeDir, 'Library', 'Sounds', fileName)
...
test('macSoundName maps boolean and named sounds', () => {
  ...
  assert.equal(macSoundName('attention', 'Mezzo'), 'Basso')
  assert.equal(macSoundName('attention', 'Boop'), 'Tink')
  assert.equal(macSoundName('complete', 'attention'), 'Glass')
  assert.equal(macSoundName('attention', 'complete'), 'Funk')
...
test(
  'darwin notifier resolves custom user sounds with afplay',
  { concurrency: false },
  async () => {
    const binDir = await mkdtemp(path.join(os.tmpdir(), 'notify-native-bin-'))
    const homeDir = await mkdtemp(path.join(os.tmpdir(), 'notify-native-home-'))
    const logFile = path.join(binDir, 'notify.log')
    await createFakeMacCommands(binDir)
    const customSoundPath = await createMacSoundFixture(homeDir, 'Quartz.caf')
    await writeFile(logFile, '', 'utf8')
    const restore = await setupMacEnv(binDir, logFile, {
      osascriptOk: '1',
      afplayOk: '1',
